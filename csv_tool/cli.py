...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="csv-tool", description="Фильтр и агрегация CSV-файла"
    )
    parser.add_argument("path", type=Path, help="Путь до CSV-файла")
    subparsers = parser.add_subparsers(dest="command", required=True)
    p_where = subparsers.add_parser("where", help="Фильтр строк")
    p_where.add_argument("expr", help='e.g. "price>100"')

    p_agg = subparsers.add_parser("aggregate", help="Агрегация столбца")
    p_agg.add_argument("expr", help='e.g. "price=avg"')
